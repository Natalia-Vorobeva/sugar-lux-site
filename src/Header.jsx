...
const Header = () => {
	...
	const scrollToSection = (e, sectionId) => {
		...
		if (section) {
			...
			// Вычисляем позицию с отступом от хедера
			...
			<div className="container mx-auto px-4 sm:px-6">
				<div className="flex justify-between items-center">
					...
					<div className="flex items-center space-x-2 sm:space-x-3 ml-3">
						...
						{/* Кнопка записи на мобильных - скрывается при открытом меню */}
						{!isMenuOpen && (
							<a
								href="#contact"
								onClick={(e) => scrollToSection(e, 'contact')}
						...
						>
							<div className="relative w-6 h-6 flex flex-col items-center justify-center">
								<span className={`absolute w-5 h-0.5 bg-gray-700 rounded-full transition-all duration-300 origin-center ${isMenuOpen ? 'rotate-45 translate-y-0' : '-translate-y-1.5'
									}`}></span>
								<span className={`absolute w-5 h-0.5 bg-gray-700 rounded-full transition-all duration-300 origin-center ${isMenuOpen ? 'opacity-0 scale-0' : 'opacity-100 scale-100'
									}`}></span>
								<span className={`absolute w-5 h-0.5 bg-gray-700 rounded-full transition-all duration-300 origin-center ${isMenuOpen ? '-rotate-45 translate-y-0' : 'translate-y-1.5'
									}`}></span>
							</div>
				...
				<div className={`md:hidden overflow-hidden transition-all duration-300 ${isMenuOpen ? 'max-h-96 opacity-100 mt-4' : 'max-h-0 opacity-0'
					...
					<div className="bg-white/95 backdrop-blur-lg rounded-2xl shadow-2xl p-4 border border-pink-100">
						...
						{/* Контакты в мобильном меню (заглушки) */}
							...
							<div className="grid grid-cols-2 gap-2">
								<button
									onClick={(e) => {
										...
										alert('Функция "Позвонить" будет доступна в ближайшее время');
									}}
								...
								<button
									onClick={(e) => {
										e.preventDefault();
										// Заглушка для будущей функциональности
										alert('Функция WhatsApp будет доступна в ближайшее время');
									}}
